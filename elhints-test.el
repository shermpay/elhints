...
(ert-deftest default-filter-function-test ()
  ...
  (ert-with-test-buffer (:name "keep but hide arg that matches name")
	(insert "(mapc (lambda ()) sequence)")
	(let* ((elhints-display-min-num-args 2)
		   (it (=--call-infos-iter))
		   (call-info (iter-next it))
		   (arg-info-vec (=-call-info-arg-info-vec call-info)))
	  (should call-info)
	  (should (=-default-filter-function call-info))
	  (should (=-arg-info-show (aref arg-info-vec 0)))
